...
class PeftModelForCausalLMShared(PeftModelForCausalLM):
    ...
    def __init__(self, model, peft_config: PeftConfig, adapter_name="default"):
        ...
        # self.lora_task_embedding.update(nn.ModuleDict({adapter_name: nn.Linear(2048, self.te_dim)}))
    ...
    def forward(self, 
        ...
        if kwargs["task_id"] is not None:
            ...
            kwargs["task_weight"] = expert_weight
                    #     else:
            # expert_weight = self.lora_gate[self.active_adapter](self.lora_task_embedding[self.active_adapter](task_id))
            # kwargs["task_id"] = expert_weight
            # kwargs["task_id"] = "zhanweifu"   kwargs["task_id"] = expert_weight  原论文使用这里的expert weight作为后续所有层门控，作为专家权重，这里修改为

        # lhy_add gate and lora_task_embedding       这里加是为了用来做sentence_pooling的方案
        # kwargs['lora_gate'] = self.lora_gate
        # kwargs['lora_task_embedding'] = self.lora_task_embedding
