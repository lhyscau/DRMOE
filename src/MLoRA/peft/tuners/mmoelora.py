...
# from ..utils import TRANSFORMERS_MODELS_TO_MMOELORAS_TARGET_MODULES_MAPPING
from .lora import (
    LoraConfig,
    LoraLayer,
    LoraModel,
...
class MMOELoraLinear(nn.Linear, MMOELoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.sl_dim = kwargs.pop("sent_linear_dim", True)   #lhy_add    句子表示经过一个linear层的维度
        ...
        self.lora_sent_linear = nn.ModuleDict({})
        ...
        # self.lora_gate.update(nn.ModuleDict({adapter_name: Gate(self.te_dim, self.expert_num)}))    #只使用当前文件下的Gate类
        #独立门控的设置
        self.lora_sent_linear.update(nn.ModuleDict({adapter_name: nn.Linear(768, self.sl_dim)}))
        # self.lora_task_embedding.update(nn.ModuleDict({adapter_name: nn.Linear(768, self.te_dim)}))
        self.lora_gate.update(nn.ModuleDict({adapter_name: Gate(self.sl_dim, self.expert_num)}))
