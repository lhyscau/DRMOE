...
class MMOELoraConfigS(LoraConfig):
    ...
    sent_linear_dim: int = field(default=32)
...
class MMOELoraModelS(MMOELoraModel):
    ...
    def _find_and_replace(self, adapter_name):
        ...
        kwargs = {
            ...
            "sent_linear_dim": lora_config.sent_linear_dim
        }
...
class MMOELoraLinearS(MMOELoraLinear):
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        if kwargs["sent_rep"] is not None:
            sent_rep = kwargs["sent_rep"]
            if len(sent_rep.shape) < 2:  # one-hot
                sent_weight = self.lora_gate[self.active_adapter](self.lora_sent_linear[self.active_adapter](sent_rep))
            else:
                sent_weight = self.lora_gate[self.active_adapter](self.lora_sent_linear[self.active_adapter](sent_rep))
        ...
        if self.disable_adapters:   # No adapter
            ...
            for i in range(self.expert_num):
                result += ( # lora process
                    ...
                    * self.scaling[self.active_adapter]
                    # * kwargs["task_weight"][..., i].unsqueeze(-1).unsqueeze(0).transpose(0, 1)
                    * (0.5 * kwargs["task_weight"][..., i].unsqueeze(-1).unsqueeze(0).transpose(0, 1)
                      + 0.5 * sent_weight[..., i].unsqueeze(-1).unsqueeze(0).transpose(0, 1))
                )


                    #* expert_weight[..., i].unsqueeze(-1).unsqueeze(0) #原始
                    # * sent_weight[..., i].unsqueeze(-1).unsqueeze(0).transpose(0, 1)  #lhy_modify predict时lora改变为bsz，seq_len, dim，而不是seq_len, bsz, seq_len
        else:
            result = F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias)

        result = result.to(previous_dtype)

        return result, sent_weight
